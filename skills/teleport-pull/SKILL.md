...
2. **List machine branches**: Run `node "${CLAUDE_PLUGIN_ROOT}/dist/cli.js" hub-machines --hub-path <hubPath>`. This returns branches (each branch = one machine). Use `AskUserQuestion` (single-select) to let the user pick a branch. Options: each machine name with last-push timestamp as description, plus "main (merged union of all machines)".
