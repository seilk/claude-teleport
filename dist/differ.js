...
function diffMarketplaces(source, target) {
    const added = [];
    const removed = [];
    const modified = [];
    const unchanged = [];
...
function buildSummary(added, modified, removed) {
...
export function diff(source, target) {
    ...
    // Diff keybindings
    diffSingleFile(source.keybindings, target.keybindings, "keybindings", "keybindings.json", { added, removed, modified, unchanged });
    // Diff statusline script
    diffSingleFile(source.statuslineScript, target.statuslineScript, "statuslineScript", source.statuslineScript?.relativePath ?? target.statuslineScript?.relativePath ?? "statusline-command.sh", { added, removed, modified, unchanged });
    return { added, removed, modified, unchanged, summary: buildSummary(added, modified, removed) };
}
function diffSingleFile(source, target, category, relativePath, buckets) {
    if (source && !target) {
        buckets.added.push({
            category,
            relativePath,
